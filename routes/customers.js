...
router.post('/', async (req, res) => {
  try {
    ...
    if (!full_name || !email) return res.status(400).json({ message: 'Nome completo e email são obrigatórios' });

    const existingCustomer = await customerModel.getByEmail(email);
    if (existingCustomer) return res.status(400).json({ message: 'Email já cadastrado' });

    const customerId = await customerModel.create({ full_name, email, phone, address });
